...
def merge_and_deduplicate(processed_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    When a duplicate is found, its spans are merged into the existing entity.

    Args:
        processed_results: A list of dictionaries, where each dictionary contains
                           the extracted and normalized entities for a single passage.

    Returns:
        A single dictionary containing deduplicated lists of entities for the entire document.
    """
    logger.info("Starting entity merging and deduplication for the entire document...")

    # This will hold the final merged entities, e.g., {'compounds': {'CHEBI:1234': {entity_data}}}
    merged_entities_store = defaultdict(dict)

    # # Define which fields to use as a unique key for each entity type.
    # # The order determines priority. Falls back to name if none are present.
    ...
    for passage in processed_results:
        for entity_type, entities in passage.items():
            ...
            for entity in entities:
                ...
                dedupe_key = entity.get("name", "").lower()
                
                if not dedupe_key:
                    continue # Cannot process entity without a name or ID

                # If we've already seen this entity, merge the spans
                ...
                # Otherwise, add it as a new unique entity
                else:
    ...
    for entity_type, entities_dict in merged_entities_store.items():
        ...
        # Optional: Sort spans for cleaner output
        for entity in final_merged_entities[entity_type]:
            if "spans" in entity and entity["spans"]:
                # Sort by start position and remove duplicate spans
