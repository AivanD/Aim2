...
def normalize_compounds_with_pubchem(processed_results: List[Dict[str, Any]], MAX_ATTEMPTS=10) -> List[Dict[str, Any]]:
    ...
    for result in processed_results:
        ...
        for compound in result["compounds"]:
            ...
            # Memoization for CID lookup
            if original_name in name_to_cid:
                first_cid = name_to_cid[original_name]
                # logger.info(f"found {original_name} in cache with CID {first_cid}")
            else:
                first_cid = None
                for attempt in range(MAX_ATTEMPTS):
                    try:
                        ...
                        if cids:
                            # add the cid to the dictionary for memoization
                            first_cid = cids[0]
            ...
            if first_cid:
                ...
                # Memoization for SMILES/INChIKey lookup
                if first_cid in cid_to_properties:
                    ...
                    for attempt in range(MAX_ATTEMPTS):
                        try:
                            ...
                            logger.error(f"Error querying PubChem for CID {first_cid}: {e}")
                            break
