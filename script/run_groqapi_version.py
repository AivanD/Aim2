        ...
        if filename.endswith('.xml'):
            ...
            if not os.path.exists(raw_ner_output_path):   
                ...
                for passage_text, passage_offset in passages_w_offsets:
                    ...
                    # OPTION 1: API (async)
                    task = process_passage_for_ner(semaphore, passage_text, GROQ_client)
                    tasks.append(task)
            ...
            if not os.path.exists(raw_re_output_path):
                ...
                # Limit concurrency to 25 to avoid rate limits.
                re_semaphore = asyncio.Semaphore(25)
            ...
            if not os.path.exists(processed_re_output_path):
                ...
                semaphore_self_evaluation = asyncio.Semaphore(25)
                tasks = []
                ...
                for rel in relations_to_evaluate:
                    ...
                    # OPTION 1:API (async)
                    task = process_for_re_evaluation(semaphore_self_evaluation, rel.model_dump(), GROQ_client)  
                    tasks.append(task)
