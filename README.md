...
| **Self-verification**       | Add a cheap, local LLM **yes/no verifier** (e.g., Outlines + a small model) with typed outputs (`Literal["yes","no"]`. Can be done using GPT as well.).             | Built-in **self-verification** pass (LLM answers yes/no) with kNN-retrieved demos; reduces over-prediction/hallucination. [[Outlines][5]]                                                                |
